...
function getRelatives(declarations) {
  return _.reduce(declarations, (Result, declaration) => {
    const relatives = _.reduce(_.values(_.get(declaration, 'step_2')), (result, relative) => {
      const integralRelative = `${relative.subjectRelation} ${relative.lastname} ${relative.firstname} ${relative.middlename}`;
...
function getStepCoowners(declaration, step) {
  return _.reduce(_.values(_.get(declaration, step)), (result, belonging) => {
    const coownersOfOneBelonging = _.map(_.values(_.get(belonging, 'rights')), coowner => `${coowner.ua_lastname} ${coowner.ua_firstname} ${coowner.ua_middlename}`);
...
function saveCSV(judges, declarations) {
  const result = json2csv({ data: judges, fields: COLUMNS });
